...
use ggez::{
    ...
    Context, GameError, GameResult, glam::IVec2, mint::Point2,
};

use super::instances::{LayerContent, object::ObjectType, floor::FloorType, wall::Wall};
...
impl Resources {
    ...
    pub fn initialize(&mut self, ctx: &Context) -> GameResult {
        ...
        self.draw_id.insert(
            DrawId::Box,
            (Images::Box, Rect::new(0.0, 0.0, 16.0, 16.0)),
        );
        // self.draw_id.insert(
        //     DrawId::TeleBox, 
        //     ()
    ...
    pub fn draw_content(&self, canvas: &mut Canvas, content: LayerContent
    ...
    pub fn draw_wall(&self, canvas: &mut Canvas, wall: Wall, mut draw_param: DrawParam) -> GameResult {
        let draw_id_option = wall.right.map(|wl| {
            match wl.opened {
        ...
        let draw_id_option = wall.down.map(|wl| {
            match wl.opened {
